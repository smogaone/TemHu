...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Firmware-2.0.2-brightgreen.svg" alt="Firmware">
...
At the heart of the display are two counter-rotating arcs. They move in opposite directions, creating a calm, balanced composition. Their color shifts gradually through **WHO and ASHRAE Standard 55** health thresholds: a cool blue when values are outside the comfort zone (too cold / too dry), drifting through mint-green in the ideal range, into warm amber and coral as conditions become uncomfortable.
...
- 🔄 **Automatic page cycling** — rotates through all 3 pages on its own. The page switches every 15 seconds, triggered at the moment the arcs pass the 12 o'clock position for a seamless, natural transition. Manual swipe always works too.
- 📊 **Live readings** — temperature, humidity & clock always up to date
- 🎨 **Color-coded arc ring** — smooth color transitions based on **WHO & ASHRAE Standard 55** thresholds:
  - 🌡️ Temperature: blue (<16 °C) → mint-green (18–22 °C, optimal) → amber (22–26 °C) → lavender (>26 °C) — warning activates below 16 °C or above 26 °C
  - 💧 Humidity: blue (<30 %, too dry) → mint-green (40–60 %, optimal) → amber (>60 %) → coral (>70 %, mold risk) — warning activates below 30 % or above 65 %
- 📈 **24h history graph** — 96 data points × 15 min, auto-scaling Y-axis
- 🌐 **Full web interface** — all settings configurable at `http://temhu-macro.local`, no app needed
- 📡 **Home Assistant integration** — optional, via MQTT auto-discovery (no YAML needed, entities appear automatically)
...
Download the latest factory firmware from the **GitHub Releases** page:
👉 **https://github.com/smogaone/TemHu/releases**

| File | Purpose |
|:---|:---|
| `temhu-macro-firmware.factory(x.x.x).bin` | Full factory image — bootloader + partitions + firmware |

> [!NOTE]
> Always use the latest available version from the Releases page.
...
**Navigation:** Swipe left/right · Auto-cycles every 15 seconds — page switches at the moment the arcs pass the 12 o'clock position for a seamless, natural transition

**24h History Graph:** 96 points × 15 min = 24 h · Y-axis auto-scales · builds up gradually after startup

<details>
<summary>🎨 Animation Ring Color Coding (WHO & ASHRAE Standard 55)</summary>

The rotating arc ring transitions color smoothly based on the current reading, following **WHO and ASHRAE Standard 55** health and comfort thresholds.

**Humidity:**

| Range | Color | Meaning | Warning |
|:---|:---|:---|:---|
| < 30% | 🔵 Light blue `#8BC5E8` | Too dry | ⚠️ Yes |
| 30 – 40% | Light blue → Mint green | Transition | |
| 40 – 60% | 🟢 Mint `#7EDCB5` → Sage `#A8D8A8` | Optimal (ASHRAE 55) | |
| 60 – 65% | Sage → 🟡 Amber `#F5C77E` | Elevated | |
| > 65% | 🟡 Amber → 🔴 Coral `#E88B8B` | Too humid | ⚠️ Yes |
| > 70% | 🔴 Coral `#E88B8B` | Mold risk | ⚠️ Yes |

**Temperature:**

| Range | Color | Meaning | Warning |
|:---|:---|:---|:---|
| < 16°C | 🔵 Light blue `#8BC5E8` | Too cold | ⚠️ Yes |
| 16 – 18°C | Light blue → Mint green | Transition (WHO min: 18 °C) | |
| 18 – 22°C | 🟢 Mint `#7EDCB5` → Sage `#A8D8A8` | Optimal | |
| 22 – 26°C | Sage → 🟡 Amber `#F5C77E` | Warm | |
| > 26°C | 🟣 Lavender `#B39DDB` | Too warm | ⚠️ Yes |
...
temhu-macro/sensor/hum_min_today/state    → Humidity minimum today in %
temhu-macro/sensor/hum_max_today/state    → Humidity maximum today in %
temhu-macro/sensor/temp_min_today/state   → Temperature minimum today in °C
temhu-macro/sensor/temp_max_today/state   → Temperature maximum today in °C
temhu-macro/sensor/esp_temp/state         → ESP32 chip temperature in °C
temhu-macro/sensor/wifi_signal/state      → Wi-Fi signal in dBm
temhu-macro/sensor/uptime/state           → Formatted uptime
```

All sensors are registered via **HA auto-discovery** — they appear automatically under **Settings → Devices & Services → MQTT** without any manual configuration.
