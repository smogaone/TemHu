...
| **Sensor connection** | SH1.0 4-pin I2C | 8-pin header |
| **GPIO header (no solder)** | ✅ GPIO 38/39 (fw 2.0.0+) | — |
...
|:---|:---|:---|
| **Tem(H)u Nano** — Waveshare ESP32-S3 Touch AMOLED 1.43" | [waveshare.com ↗](https://www.waveshare.com/product/arduino/displays/amoled/esp32-s3-touch-amoled-1.43.htm) | [amazon.de ↗](https://amzn.eu/d/0d726VEU) |
...
### Tem(H)u Nano — SH1.0 I2C Connector (GPIO 47/48)
...
> [!NOTE]
> From **firmware 2.0.0** onwards, the 2×14-pin GPIO header (GPIO 38 SCL / GPIO 39 SDA) is supported as a second I2C bus — no soldering required.
...
> [!NOTE]
> **This is the initial release — v2.0.0.** All four firmware files are provided in this repository under [`/firmware`](firmware/).

Each model comes with two files:

| File | Purpose | When to use |
|:---|:---|:---|
| `temhu-nano-firmware.factory(2.0.0).bin` | Full factory image — bootloader + partitions + firmware | **First flash only** — via USB |
| `temhu-nano-firmware.ota(2.0.0).bin` | Firmware only — no bootloader | **Updates** — via web interface OTA |
...
4. Click **"Install"** → select the **Factory** `.bin` for your model
...
1. Connect to **`Tem(H)u Nano`** or **`Tem(H)u Macro`** — no password required
...
1. Open `http://temhu-nano.local` or `http://temhu-macro.local`
2. Go to **"OTA Update"** → upload the matching **OTA** `.bin` file
...
temhu-nano/sensor/wifi_signal/state    → Wi-Fi signal in dBm
